...
uniform sampler2D secondAlbedo;
uniform sampler2D secondNormal;
uniform sampler2D secondORM;
...
void main()
{
    vec4 tMap = texture(terrainMap, UVN);
    vec4 albedo = pow(texture(albedoTexture, UV) * color,vec4(2.2,2.2,2.2,1.0));//Texture gamma correction
    albedo = mix(albedo, pow(texture(secondAlbedo, UV) * color,vec4(2.2,2.2,2.2,1.0)) , tMap.r);
    float shadow = 1.0;
    ...
    orm = mix(orm, texture(secondORM, UV).rgb, tMap.r);
    ...
    normal = mix(normal, texture(secondNormal, UV).xyz, tMap.r);
    normal = normalize((normal * 2.0 - 1.0) * normalStrength);//Normalize normal map coefficients
    ...
    //Ambient
    vec4 ambient = ambientIrradiance(environmentCubemap, normal, albedo, roughness, metallic, vDir) * environmentStrength * ao * (1.0 - pow(tMap.b, 2.0));//Calculate simple ambient color using ambient cubemap
