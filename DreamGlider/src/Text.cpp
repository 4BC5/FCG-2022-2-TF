...
void Text::TextRendering_Init(Renderer *renderer)
{
    GLuint sampler;
...
Text::Text(Renderer* renderer)//(Window* window, Camera* cam, Node* root, Camera* directional) : Renderer(window, cam, root, directional)
{
    //ctor
...
void Text::TextRendering_LoadShader(const GLchar* const shader_string, GLuint shader_id)
{
...
void Text::TextRendering_PrintString(GLFWwindow* window, const std::string &str, float x, float y, float scale)
{
    //float scale = 1.0f;
    scale *= textscale;
    int width, height;
...
float Text::TextRendering_LineHeight(GLFWwindow* window)
{
    int width, height;
    glfwGetWindowSize(window, &width, &height);
...
float Text::TextRendering_CharWidth(GLFWwindow* window)
{
    int width, height;
    glfwGetWindowSize(window, &width, &height);
...
void Text::TextRendering_ShowFramesPerSecond(GLFWwindow* window)
{
